...
# 1 : [1, yes, post, two_head, grad_accum=10]
python3 -m memory_profiler train_seq2seq.py \
    --model_type "bart_mult_heads_2" \
    ...
    --num_train_epochs 2 \
    --learning_rate 1e-5 \
    --output_dir "/scratch/tathagato/overlap_supervision_div_loss_1_pre_lm_loss_two_head_grad_acc_10_lr_1e5_cnn" \
    --overwrite_output_dir \
    --num_decoder_layers_shared 8 \
    --divergence_loss "cosine" \
    --divergence_weight 1 \
    ...
    --use_two_head_distance_loss \
    --current_epoch 0 \
    --experiment_name "overlap_supervision_div_loss_1_pre_lm_loss_two_head_grad_accum_10_lr_1e-5_cnn" 
if [[ $? = 0 ]]; then
	echo "success"
else
	echo "failure"
